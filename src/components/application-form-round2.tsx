...
const schema = z
  .object({
    ...
    discordUsername: z.string().optional(), // Made optional since it's pre-filled from Round 1
...
interface ApplicationFormRound2Props {
  requireDiscordJoin?: boolean;
  onSuccess?: () => void;
  onSubmitted?: () => void;
  round1ApplicationId?: string;
  prefillData?: {
    linkedin?: string;
    founderName?: string;
    whatBuilding?: string;
...
export function ApplicationFormRound2({
  requireDiscordJoin = false,
  onSuccess,
  onSubmitted,
  round1ApplicationId,
  prefillData,
}: ApplicationFormRound2Props) {
  const [mustJoinDiscord] = useState(requireDiscordJoin);
  const utils = trpc.useUtils();
  const create = trpc.application.createRound2.useMutation({
    onSuccess: async () => {
      toast.success("Round 2 application submitted, check your inbox");
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      screening: {
        ...defaultScreeningValues,
        founder: {
          ...defaultScreeningValues.founder,
          founder_name: prefillData?.founderName || "",
          founder_contact: prefillData?.linkedin || "",
        },
        company: {
          ...defaultScreeningValues.company,
          company_description: prefillData?.whatBuilding || "",
        },
      },
      discordUsername: prefillData?.discordUsername || "",
  ...
  async function onSubmit(values: FormValues) {
    ...
    await create.mutateAsync({
      round1ApplicationId: round1ApplicationId!,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Application - Round 2</CardTitle>
        <p className="text-sm text-muted-foreground">
          Congratulations! You've been approved for Round 2. Please complete the full application below.
        </p>
        {prefillData && (
          <p className="text-xs text-muted-foreground">
            We've pre-filled some information from your Round 1 application. Please review and update as needed.
          </p>
        )}
      </CardHeader>
      <CardContent>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <ApplicationScreeningFields
            ...
            readonlyLinkedIn={true}
            linkedinValue={prefillData?.linkedin || ""}
            founderExtras={
          ...
          <section className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Discord username carried over from your Round 1 application.
              </p>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  id="discordUsername"
                  value={prefillData?.discordUsername || ""}
                  disabled
                  className="bg-muted/50"
                  placeholder="yourname"
                />
                <p className="text-xs text-muted-foreground">
                  Carried over from Round 1. Email support if you need to change this.
                </p>
              </div>
            </div>
          </section>
